...
class FiscalBookWizard(models.TransientModel):
    ...
    def check_report_xlsx(self):
        if self.type == 'purchase':
            ...
            row = 6
            col = 1
            titles = [(1, 'Nro. Op'), (2, 'Fecha Emisión Doc.'), (3, 'Nro. de RIF'), (4, 'Nombre ó Razón Social'), (5, 'Tipo Prov.'),
                      (6, 'Nro. de Factura'), (7, 'Nro. de Control'), (8, 'Nro. Nota de Crédito'), (9, 'Nro. Nota de Débito'),
                      (10, 'Tipo de Trans'), (11, 'Nro. Factura Afectada'), (12, 'Total Compras con IVA'), (13, 'Compras sin Derecho a Crédito'),
                      (14, 'Base Imponible Alicuota General'),
                      (15, '% Alicuota General'), (16, 'Impuesto (I.V.A) Alicuota General'), (17, 'Base Imponible Alicuota Reducida'),
                      (18, '% Alicuota Reducida'), (19, 'Impuesto (I.V.A) Alicuota Reducida'),
                      (20, 'Base Imponible Alicuota Adicional'),
                      (21, '% Alicuota Adicional'), (22, 'Impuesto (I.V.A) Alicuota Adicional'), (23, 'Base Imponible Alicuota General'),
            ...
            row += 1
            sheet.merge_range('L%s:T%s' % (row, row), 'Compras Internas gravadas por Alicuota General mas Alicuota Adicional ')
            ...
            if datos_compras_ajustes:
                ...
                sheet.write(row, col, 'TOTALES', formats['title'])
                col = 12
                sheet.write(row, col, '=SUM(M%s:M%s)' % (row_suma_ini_ajustes, row_suma_fin_ajustes), formats['title_number'])
                col = 13
                sheet.write(row, col, '=SUM(N%s:N%s)' % (row_suma_ini_ajustes, row_suma_fin_ajustes), formats['title_number'])
                col = 14
                sheet.write(row, col, '=SUM(O%s:O%s)' % (row_suma_ini_ajustes, row_suma_fin_ajustes), formats['title_number'])
                col = 15
                sheet.write(row, col, '', formats['title_number'])
                col = 16
                ...
                sheet.write(row, col, '', formats['title_number'])
                col = 29
                sheet.write(row, col, '=SUM(AD%s:AD%s)' % (row_suma_ini_ajustes, row_suma_fin_ajustes), formats['title_number'])
            ...
            formats = self.set_formats(workbook)
            datos_ventas = self.get_datas_ventas()
            if not datos_ventas:
                raise UserError('No hay datos disponibles')
            ...
            row += 1
            sheet.merge_range('L%s:R%s' % (row, row), 'Ventas Internas gravadas por Alicuota General mas Alicuota Adicional ')
            ...
            sheet.merge_range('S%s:U%s' % (row, row), '=SUMA(S%s:U%s)' % (row_resumen, row -1), formats['title_number'])
            sheet.merge_range('V%s:X%s' % (row, row), '=SUMA(V%s:X%s)' % (row_resumen, row -1), formats['title_number'])
            sheet.merge_range('Y%s:AA%s' % (row, row), '=SUMA(Y%s:AA%s)' % (row_resumen, row -1), formats['title_number'])
            row += 1
       ...
       if self.type == 'purchase':
            if self.date_start and self.date_end:
                ...
                purchase_book_ids = purchase_book_obj.search(
                    [('invoice_date', '>=', fecha_inicio), ('invoice_date', '<=', fecha_fin),('state','in',['posted'])])
            ...
            #        datas = self.read(self.id)[0]
                    ...
                    return self.env.ref('l10n_ve_full.report_purchase_book').report_action(self, data=data) #, config=False
                else:
    ...
    def get_datas_ventas(self):
        ...
        for rec in self:
            ...
            date_start = datetime.strftime(datetime.strptime(str(self.date_start), DEFAULT_SERVER_DATE_FORMAT),
                                           format_new)
            date_end = datetime.strftime(datetime.strptime(str(self.date_end), DEFAULT_SERVER_DATE_FORMAT),
                                         format_new)
...
class PurchaseBook(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        datos_compras_ajustes = []
        purchasebook_ids = self.env['account.fiscal.book.line'].search(
            [('fb_id','=',data['form']['book_id']), ('accounting_date', '>=', date_start.strftime(DATETIME_FORMAT)), ('accounting_date', '<=', date_end.strftime(DATETIME_FORMAT))], order='rank asc')
        emission_date = ' '
        sum_compras_credit = 0
        sum_total_with_iva = 0
        sum_vat_general_base = 0
        ...
        for h in purchasebook_ids:
            ...
            if (partner_1.company_type == 'company' or  partner_1.company_type == 'person') and (partner_1.people_type_company or partner_1.people_type_individual) and  (partner_1.people_type_company == 'pjdo' or partner_1.people_type_individual == 'pnre' or  partner_1.people_type_individual == 'pnnr'):
                ...
                if h.invoice_id:
                    ...
                    sum_total_with_iva = (h.fb_id.base_amount + h.fb_id.tax_amount) if h.emission_date >= date_start.date() else 0 # Total monto con IVA
                    sum_total_with_iva_ajustes = (h.fb_id.base_amount + h.fb_id.tax_amount) if h.emission_date < date_start.date() else 0
                     # Total monto con IVA
                    sum_vat_general_base += h.vat_general_base # Base Imponible Alicuota general
                    sum_vat_general_tax += h.vat_general_tax   # Impuesto de IVA
                    h_vat_general_base = h.vat_general_base
            ...
            if (partner_1.company_type == 'company' or  partner_1.company_type == 'person') and (partner_1.people_type_company or partner_1.people_type_individual) and  partner_1.people_type_company == 'pjnd':
                ...
                vat_additional_tax_importaciones = h.vat_additional_tax
                'Suma total compras con IVA'
                sum_total_with_iva = (h.fb_id.base_amount + h.fb_id.tax_amount) if h.emission_date >= date_start.date() else 0
                sum_total_with_iva_ajustes = (h.fb_id.base_amount + h.fb_id.tax_amount) if h.emission_date < date_start.date() else 0
                 # Total monto con IVA
                'SUMA TOTAL DE TODAS LAS ALICUOTAS PARA LAS IMPORTACIONES'
                sum_vat_general_base_importaciones += h.vat_general_base + h.vat_reduced_base + h.vat_additional_base # Base Imponible Alicuota general
