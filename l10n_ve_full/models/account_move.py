...
class AccountMove(models.Model):
    ...
    def action_post(self):
        ...
        if self.move_type in ['out_invoice', 'out_refund']:
            ...
            numeros_control_usados = self.env['account.move'].search([('move_type','in',['out_invoice', 'out_refund']),('nro_ctrl','=',self.nro_ctrl),('state','=','posted'),('id','!=',self.id)])
        ...
        monto_tax = 0
        if not self.journal_id.eliminar_impuestos:
            if self:
            ...
            if self.company_id.partner_id.islr_withholding_agent and self.partner_id.islr_withholding_agent:
                for concep in self.invoice_line_ids:
                    if concep.concept_id.withholdable == True:
                        if self.state == 'posted' and not self.islr_wh_doc_id:
                            for ilids in self.invoice_line_ids:
